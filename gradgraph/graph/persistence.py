...
if __name__ == "__main__":
    ...
    import orjson
    with open('../../examples/example_graph.json', 'r') as f: 
        json_str = orjson.loads(f.read())
    G = nx.node_link_graph(json_str, link='edges')
    pos = nx.get_node_attributes(G, 'pos')
    print(list(find_spectral_gaps(G, pos, 10)))
    # G = nx.Graph()
    # G.add_nodes_from([
    #     (8, {"t": 7., "pos": (0., 3.)}),
    #     (7, {"t": 6., "pos": (0., 2.)}),
    ...
    #     (8, 7),
    # ], weight=1.0)
    # x = compute_gudhi_persistence(G, 't')
    # print(x)
    # for i in x:
    #     print(i)
    #
