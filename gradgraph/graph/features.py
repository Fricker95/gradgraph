...
if __name__ == "__main__":
    coordinates = '/Users/fricker/Documents/Github/phd-fungi/data/raw/M2WT_200610_coordinates.txt' 
    branches = '/Users/fricker/Documents/Github/phd-fungi/data/raw/M2WT_200610_branches.txt' 
    coords = np.fromfile(
        coordinates, dtype=[("n", int), ("x", float), ("y", float), ("frame", int)]
    )
    branches = np.fromfile(branches, dtype=[("b_i",int),("u",int),("v",int)])
    branches = np.array([branches['u'], branches['v']]).T
    ...
    nodes = [
        (n, {
            "pos": [x*um_per_pixel, y*um_per_pixel],
            "t": frame * min_per_frame
        })
        for n, x, y, frame in coords
    ]
    ...
    list_ints = []
    list_hexs = []
    x = list(windowed_temporal_apical_features(G, pos, weight="t", span=5, dt=18))
    print(len(x))
